...
class MambaBlock(Module):
    def __init__(self, config: MambaConfig):
        ...
        self.conv1d = Conv1d(in_channels=config.d_inner, out_channels=config.d_inner, 
        ...
        # delta bias
        dt = ops.exp(
            init.rand(config.d_inner) * (math.log(config.dt_max) - math.log(config.dt_min)) + math.log(config.dt_min)
        ).clamp(min=config.dt_init_floor)
        inv_dt = dt + ops.log(-torch.expm1(-dt)) # inverse of softplus: https://github.com/pytorch/pytorch/issues/72759
        ...
        A = torch.arange(1, config.d_state + 1, dtype="float32").repeat(config.d_inner, 1)
        self.A_log = Parameter(ops.log(A)) # why store A in log ? to keep A < 0 (cf -torch.exp(...)) ? for gradient stability ?
    ...
    def ssm(self, x, z):
        ...
        # TODO check split
        delta, B, C = ops.split(deltaBC, [self.config.dt_rank, self.config.d_state, self.config.d_state], dim=-1) # (B, L, dt_rank), (B, L, N), (B, L, N)
    ...
    def selective_scan_seq(self, x, delta, A, B, C, D):
        ...
        h = init.zeros(x.size(0), self.config.d_inner, self.config.d_state, device=deltaA.device) # (B, ED, N)
        hs = []
    ...
    def ssm_step(self, x, h):
        ...
        A = -ops.exp(self.A_log.float()) # (ED, N) # todo : ne pas le faire tout le temps, puisque c'est indépendant de la timestep
        D = self.D.float()

        deltaBC = self.x_proj(x) # (B, dt_rank+2*N)

        # TODO check split
        delta, B, C = ops.split(deltaBC, [self.config.dt_rank, self.config.d_state, self.config.d_state], dim=-1) # (B, dt_rank), (B, N), (B, N)
        delta, B, C = self._apply_layernorms(delta, B, C)
...
class RMSNorm(Module):
    def __init__(self, d_model: int, eps: float = 1e-5, use_mup: bool = False, device=None, dtype="float32"):
        super().__init__()
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        output = x * ops.power(ops.summation(ops.power_scalar(output, 2), axes=(1,)), -1/2)
